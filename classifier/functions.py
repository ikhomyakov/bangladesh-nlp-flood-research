    ...
    if fields:
        ...
        if f_51:
            ...
            if date:
                ...
                if debug:
                    print("No Date in f_51 fields")
        else:
            if debug:
                print("No f_51 in fields")
    else:
        if debug:
            print("NO DATE")
        return None
...
def get_data_json(file, folder, debug=False):
    ...
    if filename:
        filename2 = (
            filename.replace(".ann", "").replace(".txt", "").replace(".json", "")
        )
    ...
    districts, divisions = [], []
    if "m_28" in js["metas"]:
        is_flood = js["metas"]["m_28"]["value"]
    if "m_6" in js["metas"]:
        flood_type = js["metas"]["m_6"]["value"]
    if "m_30" in js["metas"]:
        newspaper = js["metas"]["m_30"]["value"]
    if "m_57" in js["metas"]:
        flood_related = js["metas"]["m_57"]["value"]
    if "m_55" in js["metas"]:
        is_bangladesh = js["metas"]["m_55"]["value"]
    if "m_33" in js["metas"]:
        flood_climatechange = js["metas"]["m_33"]["value"]
    if "m_56" in js["metas"]:
        anomaly = js["metas"]["m_56"]["value"]

    for d in ["m_63", "m_66", "m_67", "m_68", "m_69"]:
        if d in js["metas"]:
            districts.append(js["metas"][d]["value"])

    for d in ["m_62", "m_64", "m_65"]:
        if d in js["metas"]:
            divisions.append(js["metas"][d]["value"])

    dates = []
    if "m_61" in js["metas"]:
        dates = [d.strip() for d in js["metas"]["m_61"]["value"].split(",")]
...
def load_data_tagtog(data_folder="data", save_file=None, debug=False):
    ...
    elif type(data_folder) == list:
        data_folder = data_folder
    else:
        raise Exception("Data Folder type should be str or list")
    columns = [
        "doc_id",
        "filename",
        "is_flood",
...
def load_data(data_folder="data", balance=None, debug=False):
    ...
    for fold in data_folder:
        if os.path.isfile(fold):
            if ".json" not in fold:
                raise Exception("Data File must be a json")
            data_files = [fold]
        else:
            data_files = [
                os.path.join(fold, f) for f in sorted(os.listdir(fold)) if ".json" in f
            ]
    ...
    if balance:
        if balance not in df.columns:
            raise Exception("Please enter valid column name")
        ...
        for i, entry in enumerate(balance_data):
            if new_df is None:
                ...
                new_df = pd.concat([new_df, entry[:minVal]])
        df = new_df
    return df
...
def remove_cols(df, cols_list):
    ...
    if "predict" in df.columns and "is_flood" in df.columns:
...
def get_new_predicted_data(prediction_folder="predictions"):
    ...
    is_not_flood_predicted_file = os.path.join(
        prediction_folder, "predicted_not_isflood.json"
    )
...
def save_data(df_data, save_folder="data"):
    ...
    json.dump(
        json.loads(js), open(os.path.join(save_folder, "isflood.json"), "w"), indent=2
    )

    js = data_false.to_json(orient="records")
    json.dump(
        json.loads(js),
        open(os.path.join(save_folder, "not_isflood.json"), "w"),
        indent=2,
    )
...
def make_data_ratio(
    ...
    if save_folder:
        save_file = os.path.join(save_folder, file_prefix + "data.json")
    if load_folder:
        load_file = os.path.join(load_folder, file_prefix + "data.json")
    ...
    train_true, test_true = train_test_split(
        true_data, test_size=test_size, random_state=shuffle_seed
    )
    train_false, test_false = train_test_split(
        false_data, test_size=test_size, random_state=shuffle_seed
    )

    train_df = pd.concat([train_true, train_false])
    train_df = train_df.sample(n=len(train_df), random_state=shuffle_seed).reset_index(
        drop=True
    )
    test_df = pd.concat([test_true, test_false])
    test_df = test_df.sample(n=len(test_df), random_state=shuffle_seed).reset_index(
        drop=True
    )
    ...
    if save_file:
        ...
        json.dump(
            {"train": json.loads(train_json), "test": json.loads(test_json)},
            open(save_file, "w"),
            indent=2,
        )
    return {"train": train_df, "test": test_df}
